...
function GrowwConnectionCard() {
  const { growwStatus, setGrowwStatus } = useAutoTradeStore()
  ...
  const checkStatus = useCallback(async () => {
    ...
    try {
      ...
      // Always fetch the server IP so we can show it (especially for IP registration)
      try {
        const ipRes = await fetch('/api/broker/groww?action=server-ip')
        const ipData = await ipRes.json()
        setServerIP(ipData.ip || null)
      } catch {}
    } catch (err) {
      setGrowwStatus({ connected: false, hasCredentials: false, error: 'Failed to check status' })
    } finally {
      setTesting(false)
    }
  }, [setGrowwStatus])
  ...
  const hasCreds = growwStatus?.hasCredentials
  const isIPError = growwStatus?.error?.includes('IP_NOT_REGISTERED') || growwStatus?.error?.includes('IP')
  const isAuthError = growwStatus?.error?.includes('AUTHORISATION_FAILED') || growwStatus?.error?.includes('AUTHENTICATION_FAILED') || growwStatus?.error?.includes('GA005')
  ...
  const regenerateToken = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (data.success) {
        toast.success('Access token regenerated!', { description: 'Testing connection now...' })
        // Re-check status after token regeneration
        const statusRes = await fetch('/api/broker/groww?action=status')
        const statusData = await statusRes.json()
        setGrowwStatus(statusData)
      } else {
        toast.error('Token regeneration failed', { description: data.error || 'Unknown error' })
      }
    } catch (err: any) {
      toast.error('Token regeneration failed', { description: err.message })
    } finally {
      setTesting(false)
    }
  }
  ...
  return (
    <Card className={`border ${connected ? 'border-emerald-800/30' : 'border-border/50'}`}>
      <CardHeader className="px-4 pt-4 pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-sm font-semibold flex items-center gap-2">
            ...
            <span>Groww Connection</span>
          </CardTitle>
          <div className="flex items-center gap-1.5">
            <Badge variant="outline" className={`text-[10px] h-5 px-1.5 ${
              connected ? 'text-emerald-400 border-emerald-500/20 bg-emerald-500/5' : 'text-muted-foreground'
            }`}>
              {connected ? <CheckCircle2 className="h-2.5 w-2.5 mr-0.5" /> : <XCircle className="h-2.5 w-2.5 mr-0.5" />}
              {connected ? 'Connected' : hasCreds ? (isIPError ? 'IP Issue' : 'Token Issue') : 'Not Set'}
            </Badge>
            <Button
              size="sm"
              variant="ghost"
      ...
      <CardContent className="px-4 pb-4 pt-0 space-y-2.5">
        {connected && growwStatus?.margin && (
          <div className="flex items-center justify-between p-2.5 rounded-lg bg-emerald-500/5 border border-emerald-500/10">
            ...
            <span className="text-xs font-semibold text-emerald-400">₹{(growwStatus.margin.clearCash || 0).toLocaleString()}</span>
          </div>
        )}

        {growwStatus?.error && (
          <div className="p-2.5 rounded-lg bg-red-500/5 border border-red-500/10">
            <p className="text-xs text-red-400 break-words">{growwStatus.error}</p>
            {(isIPError || isAuthError) && serverIP && (
              <div className="mt-2 p-2.5 rounded bg-amber-500/10 border border-amber-500/20">
                <p className="text-[11px] text-amber-400 font-semibold flex items-center gap-1.5">
                  <AlertTriangle className="h-3.5 w-3.5 shrink-0" />
                  {isIPError ? 'IP Registration Required' : 'Token Refresh Required'}
                </p>
                <p className="text-[10px] text-muted-foreground mt-1 leading-relaxed">
                  {isIPError 
                    ? 'Your server IP is not registered in the Groww API dashboard. Follow these steps:'
                    : 'Your access token may be invalid or expired. Try regenerating it:'}
                </p>
                {isIPError && (
                  <ol className="text-[10px] text-muted-foreground mt-1 space-y-0.5 list-decimal list-inside leading-relaxed">
                    <li>Go to <a href="https://groww.in/trade-api/keys" target="_blank" rel="noopener noreferrer" className="text-primary underline">Groww API Keys page</a></li>
                    <li>Click on your API key → <strong>Registered IPs</strong></li>
                    <li>Add this IP: <code className="bg-muted/80 px-1.5 py-0.5 rounded font-mono text-amber-400 font-bold">{serverIP}</code></li>
                    <li>Click <strong>Approve</strong> on the API key</li>
                    <li>Come back and click <strong>Regenerate Token</strong> below</li>
                  </ol>
                )}
                <div className="mt-2 flex items-center gap-1.5">
                  <span className="text-[9px] text-muted-foreground">Your Server IP:</span>
                  <code className="text-[10px] bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded font-mono text-amber-400 font-bold select-all break-all">{serverIP}</code>
                </div>
                <Button
                  size="sm"
                  className="h-8 text-xs w-full mt-2 gap-1.5 bg-amber-600 hover:bg-amber-700 text-white"
            ...
            {!isIPError && !isAuthError && growwStatus?.hint && (
              <p className="text-[10px] text-muted-foreground mt-1">{growwStatus.hint}</p>
            )}
          </div>
        )}
        ...
        >
          {connected ? 'Manage Connection' : 'Connect Groww'}
        ...
        <AnimatePresence>
          {showGuide && (
            ...
            >
              <Separator />
              <GrowwSetupInline onConnected={checkStatus} />
            </motion.div>
          )}
        </AnimatePresence>
      </CardContent>
...
// ─── Inline Groww Setup (API Key + Secret Flow) ──────────────

function GrowwSetupInline({ onConnected }: { onConnected: () => void }) {
  const [authMethod, setAuthMethod] = useState<'approval' | 'totp'>('totp')
  ...
  const { setGrowwStatus } = useAutoTradeStore()

  // Fetch server IP on mount
  useEffect(() => {
    async function getIP() {
      try {
        const res = await fetch('/api/broker/groww?action=server-ip')
        if (res.ok) {
          const data = await res.json()
  ...
  const saveCredentials = async () => {
    if (!apiKey.trim()) {
      toast.error(authMethod === 'totp' ? 'TOTP Token is required' : 'API Key is required')
      return
    }
    if (authMethod === 'approval' && !apiSecret.trim()) {
      toast.error('API Secret is required for approval flow')
      return
    }
    if (authMethod === 'totp' && !totpSecret.trim()) {
      toast.error('TOTP Secret is required for TOTP flow')
      return
    }
    ...
    try {
      const res = await fetch('/api/broker/groww', {
        ...
        body: JSON.stringify({
          ...
          apiKey: apiKey.trim(),
          apiSecret: authMethod === 'approval' ? apiSecret.trim() : undefined,
          authMethod,
          totpSecret: authMethod === 'totp' ? totpSecret.trim() : undefined,
        }),
      })
      const data = await res.json()
      ...
      if (data.success && data.autoConnected) {
        toast.success('Connected to Groww!', {
          description: authMethod === 'totp'
            ? 'Access token generated from saved TOTP secret'
            : 'Access token auto-generated from server',
        })
        setApiKey(''); setApiSecret(''); setTotpSecret(''); setTotpCode('')
        onConnected()
      } else if (data.success && data.tokenError) {
        toast.warning('Credentials saved, but token generation failed', { description: data.tokenError })
        onConnected()
      } else if (data.success) {
        toast.success('Credentials saved', { description: data.message })
        onConnected()
      } else {
    ...
    try {
      const res = await fetch('/api/broker/groww', {
        ...
        body: JSON.stringify({
          ...
          totp_code: authMethod === 'totp' && totpCode.trim() ? totpCode.trim() : undefined,
        }),
      })
      const data = await res.json()
      if (data.success) {
        toast.success('Access token generated!', { description: `Token: ${data.accessToken}` })
        setTotpCode('')
        const statusRes = await fetch('/api/broker/groww?action=status')
        const statusData = await statusRes.json()
        setGrowwStatus(statusData)
        ...
        toast.error('Token generation failed', { description: data.error })
      }
    } catch (err: any) {
      toast.error('Token generation failed', { description: err.message })
    } finally {
      setGenerating(false)
    }
  }
    ...
    <div className="space-y-2.5">
      <div className="grid grid-cols-2 gap-1 rounded-lg border border-border/50 bg-muted/20 p-1">
        <Button
          type="button"
          size="sm"
          variant={authMethod === 'totp' ? 'default' : 'ghost'}
        ...
        <Button
          type="button"
          size="sm"
          variant={authMethod === 'approval' ? 'default' : 'ghost'}
          className="h-7 text-xs"
      ...
      <div>
        <Label className="text-xs flex items-center gap-1.5">
          <Key className="h-3 w-3 shrink-0" /> {authMethod === 'totp' ? 'TOTP Token' : 'API Key'}
        </Label>
        <Input
          type="text"
          placeholder={authMethod === 'totp' ? 'Paste Groww TOTP token...' : 'Enter your Groww API Key...'}
          value={apiKey}
      ...
      {authMethod === 'approval' ? (
        <div>
          <Label className="text-xs flex items-center gap-1.5">
            <Lock className="h-3 w-3 shrink-0" /> API Secret (Hash)
          </Label>
          <Input
            type="password"
            placeholder="Enter your API Secret hash..."
            value={apiSecret}
        ...
        <>
          <div>
            <Label className="text-xs flex items-center gap-1.5">
              <Lock className="h-3 w-3 shrink-0" /> TOTP Secret
            </Label>
            <Input
              type="password"
              placeholder="Paste the TOTP secret shown by Groww..."
              value={totpSecret}
          ...
          <div>
            <Label className="text-xs flex items-center gap-1.5">
              <Shield className="h-3 w-3 shrink-0" /> Current 6-Digit TOTP Code
            </Label>
            <Input
              inputMode="numeric"
              maxLength={6}
              placeholder="Optional override from authenticator"
              value={totpCode}
      ...
      <Button size="sm" className="h-8 text-xs w-full gap-1.5" onClick={saveCredentials} disabled={saving}>
        {saving ? <Loader2 className="h-3 w-3 animate-spin" /> : <Wifi className="h-3 w-3" />}
        Save {authMethod === 'totp' ? 'TOTP' : 'API Secret'} Credentials
      </Button>
      <Separator />
      <Button size="sm" variant="outline" className="h-7 text-xs w-full gap-1.5" onClick={generateToken} disabled={generating}>
      ...
      <div className="p-2.5 rounded-lg bg-muted/30 border border-border/30 space-y-1.5">
        <div className="flex items-start gap-1.5">
          ...
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            {authMethod === 'totp'
              ? 'Use Groww Generate TOTP Token, then paste the TOTP token and secret here. The app can generate the daily access token from the saved secret.'
              : 'Access token is auto-generated from our server. Click Approve on the Groww API Keys page first, then save credentials.'}{' '}
            <a href="https://groww.in/trade-api/api-keys" target="_blank" rel="noopener noreferrer" className="text-primary underline">
              Groww API Keys page
            </a>
          </p>
        </div>
        <div className="flex items-center gap-1.5 text-[10px]">
          ...
          <code className="bg-muted px-1.5 py-0.5 rounded text-foreground font-mono text-[10px]">{serverIP}</code>
        </div>
        <div className="text-[10px] text-muted-foreground space-y-0.5 mt-1 leading-relaxed">
          {authMethod === 'totp' ? (
            <>
              <p>1. Go to <a href="https://groww.in/trade-api/api-keys" target="_blank" rel="noopener noreferrer" className="text-primary underline">Groww API Dashboard</a></p>
              <p>2. Open the key dropdown and choose <strong>Generate TOTP Token</strong></p>
              <p>3. Paste the TOTP token and TOTP secret above</p>
              <p>4. Save credentials, then generate the access token</p>
            </>
          ) : (
            <>
              <p>1. Go to <a href="https://groww.in/trade-api/api-keys" target="_blank" rel="noopener noreferrer" className="text-primary underline">Groww API Dashboard</a></p>
              <p>2. Create or select your API key</p>
              <p>3. Add server IP: <code className="bg-muted px-1 rounded">{serverIP}</code></p>
              <p>4. Click <strong>Approve</strong> on the API key</p>
              <p>5. Enter API Key &amp; Secret above, then save credentials</p>
            </>
          )}
        </div>
      </div>
