...
export function inferTradeType(input: InferTradeTypeInput): TradeType {
  ...
  if (/\b(SCALP|SCALPING|QUICK TRADE)\b/.test(text)) return 'SCALP';
  if (/\b(BTST|BUY TODAY SELL TOMORROW)\b/.test(text)) return 'BTST';
  if (/\b(INTRADAY|INTRA DAY|DAY TRADE|DAYTRADING|MIS)\b/.test(text)) return 'INTRADAY';
  if (/\b(POSITIONAL|POSITION TRADE|HOLD FOR|1W|WEEKLY|MONTHLY)\b/.test(text)) return 'POSITIONAL';
  if (/\b(SWING|BREAKOUT|RETEST|1D|DAILY)\b/.test(text)) return 'SWING';
  if (/\b(DELIVERY|CNC|INVEST|INVESTMENT|LONG TERM|LONG-TERM|PORTFOLIO)\b/.test(text)) return 'DELIVERY';
